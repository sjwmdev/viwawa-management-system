                    ...
                    <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
                        ...
                        @can('admin.attendance.create')
                            <div class="btn-group btn-group-md ml-auto" role="group">
                                <a href="{{ route('admin.attendance.create') }}" class="btn btn-outline-light"><i
                                            ...
                                            <tr>
                                                ...
                                                @can('admin.attendance.update')
                                                    <td>
                                                        @if (isset($attendances[$member->id]))
                                                            <form
                                                                action="{{ route('admin.attendance.update', $attendances[$member->id]->id) }}"
                                                                method="POST" class="update-form">
                                                                @csrf
                                                                @method('PUT')
                                                                <select name="present" class="form-control update-attendance"
                                                                    data-id="{{ $attendances[$member->id]->id }}">
                                                                    <option value="1"
                                                                        {{ $attendances[$member->id]->present ? 'selected' : '' }}>
                                                                        Alihudhuria</option>
                                                                    <option value="0"
                                                                        {{ !$attendances[$member->id]->present ? 'selected' : '' }}>
                                                                        Hakuhudhuria</option>
